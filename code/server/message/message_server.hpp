...
namespace chat_ns
{
    class MessageServiceImpl : public chat_ns::MsgStorageService
    {
    public:
        MessageServiceImpl(
                                   ...
                                   ::google::protobuf::Closure *done)
        {
            ...
            std::string stime = timestampToDate(request->start_time());
            std::string etime = timestampToDate(request->over_time());
            // 2. 从数据库中进行消息查询
            vector<Message> msg_lists;

            bool ret = _mysql_message->getMessagesBySessionId(chat_ssid, msg_lists, stime, etime);
            if (ret == false)
            {
                response->set_request_id(rid);
            ...
            ret = _GetFile(rid, file_id_lists, file_data_lists);
            if (ret == false)
            {
                                  ...
                                  ::google::protobuf::Closure *done)
        {
            ...
            // 2. 从数据库获取最近的消息元信息
            vector<Message> msg_lists;
            bool ret = _mysql_message->getMessagesBySessionId(chat_ssid, msg_lists, msg_count);
            if (ret == false)
            {
                response->set_request_id(rid);
                response->set_success(true);
        ...
        void onMessage(const char *body, size_t sz)
        {
            ...
            ret = _mysql_message->createMessage(msg);
            if (ret == false)
            {
    ...
    private:
        ...
        {
            // 将 int64_t 时间戳转换为 std::time_t
            std::time_t time = static_cast<std::time_t>(timestamp);

            // 将 std::time_t 转换为 tm 结构
            std::tm *tm_time = std::localtime(&time);

            // 创建一个字符串流来存储格式化的日期字符串
            std::ostringstream oss;

            // 使用 std::put_time 进行格式化输出
            // 例如格式化为 "YYYY-MM-DD HH:MM:SS"
