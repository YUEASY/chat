...
#include "es_operations.hpp"    // es数据管理客户端封装
#include "mysql_operations.hpp" // mysql数据管理客户端封装
...
namespace chat_ns
{
    class FriendServiceImpl : public chat_ns::FriendService
    {
    public:
        FriendServiceImpl(
            ...
            const std::string &message_service_name) : _es_user(std::make_shared<ESUser>(es_client)),
                                                       _mysql_apply(std::make_shared<FriendApplyTable>()),
                                                       _mysql_chat_session(std::make_shared<ChatSessionTable>()),
                                                       _mysql_chat_session_member(std::make_shared<ChatSessionMemberTable>()),
                                                       _mysql_relation(std::make_shared<RelationTable>()),
                                   ...
                                   ::google::protobuf::Closure *done)
        {
            ...
            bool ret = _mysql_relation->getFriendIds(uid, friend_id_lists);
            ...
            ret = GetUserInfo(rid, user_id_lists, user_list);
            if (ret == false)
            {
                                  ...
                                  ::google::protobuf::Closure *done)
        {
            ...
            // 2. 从好友关系表中删除好友关系信息
            bool ret = _mysql_relation->removeFriend(uid, pid);
            if (ret == false)
            {
            ...
            ret = _mysql_chat_session->deleteChatSession(uid, pid);
            if (ret == false)
            {
                               ...
                               ::google::protobuf::Closure *done)
        {
            ...
            // 2. 判断两人是否已经是好友
            bool ret;
            _mysql_relation->areFriends(uid, pid, ret);
            if (ret == true)
            {
            ...
            // 3. 当前是否已经申请过好友
            _mysql_apply->applyExists(uid, pid, ret);
            if (ret == true)
            {
            ...
            ret = _mysql_apply->addFriendApply(ev);
            if (ret == false)
            {
                                      ...
                                      ::google::protobuf::Closure *done)
        {
            ...
            // 2. 判断有没有该申请事件
            bool ret;
            _mysql_apply->applyExists(pid, uid, ret);
            if (ret == false)
            {
            ...
            ret = _mysql_apply->deleteFriendApply(pid, uid);
            if (ret == false)
            {
            ...
            if (agree == true)
            {
                ret = _mysql_relation->addFriend(uid, pid);
                if (ret == false)
                {
                ...
                cssid = Utils::uuid();
                ChatSession cs(cssid, "", 1);
                ret = _mysql_chat_session->createChatSession(cs);
                if (ret == false)
                {
                ...
                ret = _mysql_chat_session_member->addSessionMembers(mlist);
                if (ret == false)
                {
                    LOG_ERROR("{}- 没有找到{}-{}对应的好友申请事件！", rid, pid, uid);
                    return err_response(rid, "没有找到对应的好友申请事件!");
                                  ...
                                  ::google::protobuf::Closure *done)
        {
            ...
            std::vector<std::string> friend_id_lists;
            _mysql_relation->getFriendIds(uid, friend_id_lists);
                                               ...
                                               ::google::protobuf::Closure *done)
        {
            ...
            _mysql_apply->getFriendAppliesByUserId(uid, res);
            std::unordered_set<std::string> user_id_lists;
            for (auto &id : res)
            {
                user_id_lists.insert(id);
                                        ...
                                        ::google::protobuf::Closure *done)
        {
            ...
            // 3. 从数据库中查询出用户的群聊会话列表
            std::vector<GroupChatSession> gc_list;
            _mysql_chat_session->getChatSessionsByUserIdType2(uid,gc_list);
                                       ...
                                       ::google::protobuf::Closure *done)
        {
            ...
            ChatSession cs(cssid, cssname, 2);
            bool ret = _mysql_chat_session->createChatSession(cs);
            if (ret == false)
            {
            ...
            ret = _mysql_chat_session_member->addSessionMembers(member_list);
            if (ret == false)
            {
                                          ...
                                          ::google::protobuf::Closure *done)
        {
            ...
            _mysql_chat_session_member->getMembersBySession(cssid, member_id_lists);
            std::unordered_set<std::string> uid_list;
            for (const auto &id : member_id_lists)
            {
                uid_list.insert(id.user_id);
            }
            // 3. 从用户子服务批量获取用户信息
            std::unordered_map<std::string, UserInfo> user_list;
    ...
    private:
                          ...
                          const std::string &cssid, MessageInfo &msg)
        {
            ...
            chat_ns::MsgStorageService_Stub stub(channel.get());
                         ...
                         std::unordered_map<std::string, UserInfo> &user_list)
        {
            ...
            brpc::Controller cntl;
            chat_ns::UserService_Stub stub(channel.get());
    ...
    public:
        ...
        void make_rpc_server(uint16_t port, int32_t timeout, uint8_t num_threads)
        {
            ...
            FriendServiceImpl *friend_service = new FriendServiceImpl(_es_client, _mm_channels, _user_service_name, _message_service_name);
